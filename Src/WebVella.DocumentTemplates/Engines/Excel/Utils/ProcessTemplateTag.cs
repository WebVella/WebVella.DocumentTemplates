...
public static partial class WvExcelFileEngineUtility
{
	...
	private static string? generateExcelFunction(
		WvTemplateTagResultList input,
		DataTable dataSource,
		WvExcelFileTemplateProcessResult result,
		WvExcelFileTemplateProcessResultItem resultItem,
	...
	{
		if (input.Tags.Count != 1) return null;
		if (input.Tags[0].Type != WvTemplateTagType.ExcelFunction) return null;
		var tag = input.Tags[0];
		string? excelFunctionString = null;
			...
			case "sum":
				{
						...
						&& !String.IsNullOrWhiteSpace(tag.FullString))
					{
						...
						if (rangeList.Count > 0)
							excelFunctionString = $"=SUM({String.Join(",", rangeList)})";
					}
					break;
				}
			default:
