...
public partial class WvExcelFileEngineUtility
{
		...
		foreach (var resultWorksheet in resultItem.Result.Worksheets)
		{
				...
				x.Worksheet?.Position == resultWorksheet.Position))
			{
				...
				while (queue.Count > 0)
				{
					...
					{
						resultItem.Contexts.Add(new WvExcelFileTemplateProcessContext
						{
							TemplateContextId = templateContext.Id,
							ExcelCellError = WvExcelFileTemplateProcessResultItemContextError.ProcessError,
							ExcelCellErrorMessage = ex.Message,
		...
		WvExcelFileTemplateProcessResultItem resultItem, int limit)
	{
		...
		if (resultItem.ContextProcessLog[templateContext.Id] > limit)
		{
			...
			{
				TemplateContextId = templateContext.Id,
				ExcelCellError = WvExcelFileTemplateProcessResultItemContextError.DependencyOverflow,
		...
		WvExcelFileTemplateProcessResultItem resultItem)
	{
		...
		{
			var dependency = resultItem.Contexts.FirstOrDefault(x => x.TemplateContextId == contextId);
			if (dependency is not null && dependency.ExcelCellError is not null)
			{
				resultItem.Contexts.Add(new WvExcelFileTemplateProcessContext
				{
					TemplateContextId = templateContext.Id,
					ExcelCellError = dependency.ExcelCellError
				});
				return true;
			}
		}
		...
		DataTable dataSource, CultureInfo culture)
	{
			...
			rowNum <= templateLastAddress.RowNumber; rowNum++)
		{
				...
				colNum <= templateLastAddress.ColumnNumber; colNum++)
			{
				...
				var resultItemContext = new WvExcelFileTemplateProcessContext
				{
					TemplateContextId = templateContext.Id,
