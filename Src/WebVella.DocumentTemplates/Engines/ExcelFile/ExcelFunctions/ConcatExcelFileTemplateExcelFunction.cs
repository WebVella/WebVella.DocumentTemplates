...
namespace WebVella.DocumentTemplates.Engines.ExcelFile.ExcelFunctions;
public class ConcatExcelFileTemplateExcelFunction : IWvExcelFileTemplateExcelFunctionProcessor
{
		...
		)
	{
		...
		if (rangeList.Count > 0)
			FormulaA1 = $"=CONCAT({String.Join(",", rangeList)})";
		else
		{
			HasError = true;
